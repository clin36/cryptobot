...
1] == token, "Invalid ETH to token path");
} else {
...
uint256[] memory out = uniswapRouter.getAmountsOut(tokenAmount,
path);
uint256 minOut = _calculateMinOut(out[out.length - 1]);
...
tradeHistory.push(Trade(token, tokenAmount, result[result.length -
1], block.timestamp, false));
totalTrades++;
